...
public final class DynamicNotch<Expanded, CompactLeading, CompactTrailing>: ObservableObject, DynamicNotchControllable where Expanded: View, CompactLeading: View, CompactTrailing: View {
    ...
    /// VibeBar: fixed shell width; compact and expanded share width, only height animates.
    public var islandShellWidth: CGFloat?
...
extension DynamicNotch {
    private var inPlaceExpansionAnimation: Animation {
        if islandShellWidth != nil {
            .bouncy(duration: 0.38)
        } else {
            style.conversionAnimation
        }
    }
    ...
    public func expandInPlace(on screen: NSScreen = NSScreen.screens[0]) async {
    ...
    func _expand(on screen: NSScreen = NSScreen.screens[0], skipHide: Bool) async {
        ...
        Task { @MainActor in
            if state != .hidden {
                ...
                withAnimation(self.inPlaceExpansionAnimation) {
                    self.state = .expanded
                }
            } else {
                withAnimation(self.inPlaceExpansionAnimation) {
    ...
    public func compactInPlace(on screen: NSScreen = NSScreen.screens[0]) async {
